        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="color:black">{{$invoice->prices()->pluck('jenis')->implode(',')}}</td>
                    <td style="color:black">{{$invoice->kg}} Kg</td>
                    <td style="color:black">{{Rupiah::getRupiah($invoice->harga)}} /Kg</td>
                    <td><input type="hidden" value="{{$hitung = $invoice->kg * $invoice->harga}}">
                        <p style="color:black">{{Rupiah::getRupiah($hitung)}}</p>
                    </td>
                </tr>
                <tr>
                    <th colspan="4">Disc @if ($invoice->disc == "")
                        0 %
                        @else
                        {{$invoice->disc}} %
                        @endif </th>
                    <td style="color:black"><input type="hidden" value="{{$disc = ($hitung * $invoice->disc) / 100}}">
                        {{Rupiah::getRupiah($disc)}}</td>
                </tr>
                <tr>
                    <th colspan="4">Total Bayar</th>
                    <td style="color:black; font-weight:bold">{{Rupiah::getRupiah($invoice->harga_akhir)}}</td>
                </tr>
            </tbody>
        </table>
